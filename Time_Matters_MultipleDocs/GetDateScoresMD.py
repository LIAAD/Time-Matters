...
def GetDataScores(inverted_index, words_array, dates_array, n_contextual_window, TH, N, score_type):
    ...
    for i in range(0, len(clean_unic_array)):
        ...
        for j in range(i+1, len(clean_unic_array)):
            ...
            px_y, px, py = find_axis_data(inverted_index[Term1],
                                          inverted_index[Term2],
                                          n_contextual_window, score_type)
...
def find_axis_data(x_axis, y_axis, n_contextual_window, score_type):
    ...
    if score_type == 'ByCorpus':
        ...
        pxy, px, py = BySentences_find_data(x_axis, y_axis, n_contextual_window)
...
def BySentences_find_data(x_axis, y_axis, doc_n_contextual_window):
    count = 0
    x = 0
    y = len(([sentence for i in y_axis[2] for sentence in y_axis[2][i][2]]))
    for key in x_axis[2]:
        x += len(x_axis[2][key][2].items())
        if key in y_axis[2]:
            for sentence in x_axis[2][key][2]:
                if sentence in y_axis[2][key][2]:
                    if doc_n_contextual_window == 'full_sentence':
                        count += 1
                    else:
                        x_offset = x_axis[2][key][2][sentence][1]
                        y_offset = y_axis[2][key][2][sentence][1]
...
def Create_ContextVectorByDocSentence(term, DF, TH, Inverted_Index, Index, sentence_index, n_contextual_window):
    ...
    for x in DF_Filtered:
        try:
            ...
            if n_contextual_window != 'full_sentence':
                offset_x = Inverted_Index[x[0]][2][Index][2][sentence_index][1]
